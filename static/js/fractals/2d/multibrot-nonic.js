...
// Helper function to create UBO-aware fragment shader for nonic multibrot
function createNonicMultibrotFragmentShader(useUBO) {
  ...
  return `${uniformDeclarations}
    ...
    // Optimized function to compute z^9 for complex number z
    // z^9 = z^8 * z, so we can compute z^8 first then multiply by z
    vec2 complexNonic(vec2 z) {
        // First compute z^2
        ...
        // Finally compute z^9 = z^8 * z
        return vec2(z8.x * z.x - z8.y * z.y, z8.x * z.y + z8.y * z.x);
    }
    
    float computeFractal(vec2 c) {
        // Nonic multibrot set: z = z^9 + c
        vec2 z = vec2(0.0);
        float zx2 = 0.0;
        float zy2 = 0.0;
...
export function render(regl, params, canvas, options = {}) {
  ...
  const fragmentShader = createNonicMultibrotFragmentShader(useUBO);

  // Use createStandardDrawCommand for UBO support
  // Note: nonic multibrot uses custom shader, so we pass it directly
...
/**
 * Configuration for Nonic Multibrot fractal
 */
export const config = {
  initialSettings: {
