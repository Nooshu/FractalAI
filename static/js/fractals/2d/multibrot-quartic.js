...
// Helper function to create UBO-aware fragment shader for quartic multibrot
function createQuarticMultibrotFragmentShader(useUBO) {
  ...
  return `${uniformDeclarations}
    ...
    // Optimized function to compute z^4 for complex number z
    // z^4 = (z^2)^2, so we can use the square twice
...
export function render(regl, params, canvas, options = {}) {
  ...
  const fragmentShader = createQuarticMultibrotFragmentShader(useUBO);

  // Use createStandardDrawCommand for UBO support
  // Note: quartic multibrot uses custom shader, so we pass it directly
...
/**
 * Configuration for Quartic Multibrot fractal
 */
export const config = {
  initialSettings: {
