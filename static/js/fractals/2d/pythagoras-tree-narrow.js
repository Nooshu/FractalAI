...
// Generate vertices for the Pythagoras tree fractal with narrower spread
function generatePythagorasTreeNarrow(iterations, angle) {
  const segments = [];
  ...
  function addSquare(cx, cy, size, rotation, depth) {
    ...
    // For narrow variant, use wider rotation angles (60 degrees) which creates a narrower overall tree
    // and adjust positioning for better spread which makes the tree appear narrower/taller
    const childSize1 = topEdgeLength * angle;
    const childSize2 = topEdgeLength * (1.0 - angle);

    // Calculate positions for child squares
    // For narrow variant, adjust positioning to create more spread (taller/narrower tree)
    const spreadFactor = 1.2; // Spread squares further apart
    const child1CenterX = topLeftX + (dx * angle * spreadFactor);
    const child1CenterY = topLeftY + (dy * angle * spreadFactor);
    const child1Rotation = rotation + Math.PI / 3; // 60 degrees (creates narrower tree)

    // Second child square (right side of triangle)
    const child2CenterX = topRightX - (dx * (1.0 - angle) * spreadFactor);
    const child2CenterY = topRightY - (dy * (1.0 - angle) * spreadFactor);
    const child2Rotation = rotation - Math.PI / 3; // -60 degrees (creates narrower tree)
...
export function render(regl, params, canvas, options = {}) {
  ...
  // For narrow variant, use a wider default ratio (more balanced squares)
  // 0.0 = 0.4, 1.0 = 0.6 (narrower range, more balanced)
  const angle = 0.4 + params.xScale * 0.2;

  // Generate vertices for current iteration level
  const vertices = generatePythagorasTreeNarrow(iterationLevel, angle);
...
/**
 * Configuration for Pythagoras Tree (Narrow) fractal
 */
export const config = {
  initialSettings: {
    colorScheme: 'neon'
  ...
  // Interesting bounds for "surprise me" - Pythagoras tree narrow is always interesting
  interestingBounds: {
    offsetX: [-1, 1],
    offsetY: [-1, 1],
    zoom: [0.5, 10],
