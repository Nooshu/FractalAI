...
function generateOakTreeLSystem(iterations, angle = 30) {
  const angleRad = (angle * Math.PI) / 180;
  const segments = [];

  // L-system rules for oak tree
  // Axiom: F
  // Rules: F → F[+F]F[-F]F
  let lsystemString = 'F';
...
export function render(regl, params, canvas, options = {}) {
  ...
  // Use uXScale to control branch angle (20-40 degrees for oak)
  const angle = 20 + params.xScale * 20;

  // Generate vertices for current iteration level
  const vertices = generateOakTreeLSystem(iterationLevel, angle);
...
/**
 * Configuration for Oak Tree L-System
 */
export const config = {
  initialSettings: {
    colorScheme: 'forest',
