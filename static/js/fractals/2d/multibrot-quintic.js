...
// Helper function to create UBO-aware fragment shader for quintic multibrot
function createQuinticMultibrotFragmentShader(useUBO) {
  ...
  return `${uniformDeclarations}
    ...
    // Optimized function to compute z^5 for complex number z
    // z^5 = z^4 * z, so we can compute z^4 first then multiply by z
    vec2 complexQuintic(vec2 z) {
        // First compute z^2
        vec2 z2 = vec2(z.x * z.x - z.y * z.y, 2.0 * z.x * z.y);
        // Then compute z^4 = (z^2)^2
        vec2 z4 = vec2(z2.x * z2.x - z2.y * z2.y, 2.0 * z2.x * z2.y);
        // Finally compute z^5 = z^4 * z
        return vec2(z4.x * z.x - z4.y * z.y, z4.x * z.y + z4.y * z.x);
    }
    
    float computeFractal(vec2 c) {
        // Quintic multibrot set: z = z^5 + c
        vec2 z = vec2(0.0);
        float zx2 = 0.0;
        float zy2 = 0.0;
        
        // Unroll first few iterations for better performance
        z = complexQuintic(z) + c;
        zx2 = z.x * z.x;
        zy2 = z.y * z.y;
        if (zx2 + zy2 > 4.0) {
...
export function render(regl, params, canvas, options = {}) {
  ...
  const fragmentShader = createQuinticMultibrotFragmentShader(useUBO);

  // Use createStandardDrawCommand for UBO support
  // Note: quintic multibrot uses custom shader, so we pass it directly
...
/**
 * Configuration for Quintic Multibrot fractal
 */
export const config = {
  initialSettings: {
