...
// Generate vertices for quaternary fractal tree (4 branches)
function generateQuaternaryFractalTree(iterations, branchAngle, lengthRatio) {
  const segments = [];
  ...
  function generateBranch(x, y, angle, length, depth) {
    ...
    // Quaternary tree: four branches arranged symmetrically
    // One continues straight, one goes left, one goes right, one goes back
    ...
    generateBranch(endX, endY, angle + Math.PI, newLength * 0.7, depth + 1); // Back branch (shorter)
  }
...
export function render(regl, params, canvas, options = {}) {
  ...
  const vertices = generateQuaternaryFractalTree(iterationLevel, branchAngle, lengthRatio);
  ...
  return drawQuaternaryTree;
}

export const is2D = true;

/**
 * Configuration for Quaternary Fractal Tree
 */
export const config = {
  initialSettings: {
    colorScheme: 'cosmic'
  ...
  // Interesting bounds for "surprise me" - Quaternary fractal tree is always interesting
  interestingBounds: {
    offsetX: [-1, 1],
    offsetY: [-1, 1],
    zoom: [0.5, 10],
