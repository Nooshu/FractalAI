...
function generatePythagorasTree(iterations, angle) {
  ...
  function addSquare(cx, cy, size, rotation, depth) {
    ...
    // angle = 0.5 gives classic 45-degree Pythagoras tree
    // angle controls the ratio of the two child squares
...
export function render(regl, params, canvas, options = {}) {
  ...
  // 0.0 = 0.3, 1.0 = 0.7 (typical range for Pythagoras tree)
  const angle = 0.3 + params.xScale * 0.4;

  // Generate vertices for current iteration level
  const vertices = generatePythagorasTree(iterationLevel, angle);
...
/**
 * Configuration for Pythagoras Tree fractal
 */
export const config = {
  initialSettings: {
    colorScheme: 'neon'
