...
// Helper function to create UBO-aware fragment shader for sextic multibrot
function createSexticMultibrotFragmentShader(useUBO) {
  ...
  return `${uniformDeclarations}
    ...
    // Optimized function to compute z^6 for complex number z
    // z^6 = (z^3)^2, so we can compute z^3 first then square it
    vec2 complexSextic(vec2 z) {
        // First compute z^2
        vec2 z2 = vec2(z.x * z.x - z.y * z.y, 2.0 * z.x * z.y);
        // Then compute z^3 = z^2 * z
        vec2 z3 = vec2(z2.x * z.x - z2.y * z.y, z2.x * z.y + z2.y * z.x);
        // Finally compute z^6 = (z^3)^2
        return vec2(z3.x * z3.x - z3.y * z3.y, 2.0 * z3.x * z3.y);
    }
    
    float computeFractal(vec2 c) {
        // Sextic multibrot set: z = z^6 + c
        vec2 z = vec2(0.0);
        float zx2 = 0.0;
        float zy2 = 0.0;
...
export function render(regl, params, canvas, options = {}) {
  ...
  const fragmentShader = createSexticMultibrotFragmentShader(useUBO);

  // Use createStandardDrawCommand for UBO support
  // Note: sextic multibrot uses custom shader, so we pass it directly
...
/**
 * Configuration for Sextic Multibrot fractal
 */
export const config = {
  initialSettings: {
