...
// Helper function to create UBO-aware fragment shader for septic multibrot
function createSepticMultibrotFragmentShader(useUBO) {
  ...
  return `${uniformDeclarations}
    ...
    // Optimized function to compute z^7 for complex number z
    // z^7 = z^6 * z, so we can compute z^6 first then multiply by z
    vec2 complexSeptic(vec2 z) {
        // First compute z^2
        vec2 z2 = vec2(z.x * z.x - z.y * z.y, 2.0 * z.x * z.y);
        // Then compute z^3 = z^2 * z
        vec2 z3 = vec2(z2.x * z.x - z2.y * z.y, z2.x * z.y + z2.y * z.x);
        // Then compute z^6 = (z^3)^2
        vec2 z6 = vec2(z3.x * z3.x - z3.y * z3.y, 2.0 * z3.x * z3.y);
        // Finally compute z^7 = z^6 * z
        return vec2(z6.x * z.x - z6.y * z.y, z6.x * z.y + z6.y * z.x);
    }
    
    float computeFractal(vec2 c) {
        // Septic multibrot set: z = z^7 + c
        vec2 z = vec2(0.0);
        float zx2 = 0.0;
        float zy2 = 0.0;
...
export function render(regl, params, canvas, options = {}) {
  ...
  const fragmentShader = createSepticMultibrotFragmentShader(useUBO);

  // Use createStandardDrawCommand for UBO support
  // Note: septic multibrot uses custom shader, so we pass it directly
...
/**
 * Configuration for Septic Multibrot fractal
 */
export const config = {
  initialSettings: {
