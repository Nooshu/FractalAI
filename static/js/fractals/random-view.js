/**
 * Random Interesting View Module
 * Generates random interesting views for fractals
 * Handles validation, duplicate removal, and special cases
 */
...
/**
 * Remove duplicate points and replace with new random ones
 * @param {Array} locations - Array of location objects
 * @param {string} fractalType - Type of fractal
 * @param {Function} generateRandomInterestingPoint - Function to generate random points
 * @returns {Array} Array of unique locations
 */
export function removeDuplicatePoints(locations, fractalType, generateRandomInterestingPoint) {
  const seen = new Set();
  const unique = [];
 ...
 * @param {string} fractalType - Type of fractal
 * @param {Set} excludeSet - Set of keys to exclude
 * @returns {Object} Random point {x, y, zoom}
 */
export function generateRandomInterestingPoint(fractalType, excludeSet) {
  let attempts = 0;
  let newPoint;
...
/**
 * Get random interesting view for current fractal
 ...
 * @returns {Object} Random view {offset: {x, y}, zoom}
 */
export function getRandomInterestingView(getters) {
  const { getCurrentFractalType, getCurrentFractalModule, getParams, getCanvas } = getters;
  const fractalType = getCurrentFractalType();
  const fractalConfig = getCurrentFractalModule()?.config;
  const params = getParams();
  ...
  if (fractalConfig?.interestingPoints && fractalConfig.interestingPoints.length > 0) {
    ...
    if (fractalType === 'julia' || fractalType === 'julia-snakes') {
      ...
      for (let attempt = 0; attempt < 10; attempt++) {
        ...
        // Validate the view
        if (isValidInterestingView(offset, zoom, fractalType, getParams, getCanvas)) {
  ...
  switch (fractalType) {
    case 'mandelbrot': {
      ...
      for (let attempt = 0; attempt < 10; attempt++) {
        ...
        // Validate the view
        if (isValidInterestingView(offset, zoom, 'mandelbrot', getParams, getCanvas)) {
          return { offset, zoom };
        }
      }
    ...
    case 'multibrot': {
      ...
      for (let attempt = 0; attempt < 10; attempt++) {
        ...
        // Validate the view
        if (isValidInterestingView(offset, zoom, 'multibrot', getParams, getCanvas)) {
          return { offset, zoom };
        }
      }
